-- CRITICAL SECURITY FIXES - Phase 1: RLS Policy Fixes
BEGIN;
...
-- Add security logging triggers to sensitive tables
CREATE TRIGGER security_log_messages 
  AFTER SELECT ON public.messages 
  FOR EACH STATEMENT 
  EXECUTE FUNCTION public.log_security_event();

CREATE TRIGGER security_log_escrow 
