-- Manual application of Phase 3A migration to production
-- Run this in Supabase SQL Editor if automated push fails
...
-- Add indexes for faster queries on email tracking
...
DROP POLICY IF EXISTS "Admins can view all response tracking" ON public.email_response_tracking;
DROP POLICY IF EXISTS "Service role can insert response tracking" ON public.email_response_tracking;
DROP POLICY IF EXISTS "Service role can update response tracking" ON public.email_response_tracking;
...
WHERE table_name = 'email_response_tracking'
